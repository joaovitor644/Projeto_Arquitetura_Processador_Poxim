...
ArqResources* inicialization(){
	...
	newArq->buffer = (int32_t*)malloc(sizeof(int32_t)*100);
	newArq->term = 0;
	newArq->index = 0;
	newArq->size = 100;
	return newArq;
...
void executionInstructionTypeF(ArqResources* arq,typeF* instruction,FILE* output){
	...
	switch(instruction->opcode){
		...
		//	printf("----_---->0x%08X\n",(arq->Mem[(arq->Reg[instruction->x] + IplusSinal) >> 2]) );
		//	printf("instrução z = 0x%08X\n",arq->Reg[instruction->x] + IplusSinal );
		...
		case 0b011011:
		  ...
		  if(arq->Reg[instruction->x] + IplusSinal == 0x8888888B){
				b1 = ShiftBit(arq->term,0,8);
			 	b2 = ShiftBit(arq->term,8,8);
			 	b3 = ShiftBit(arq->term,16,8);
			 	b4 = ShiftBit(arq->term,24,8);
				uint8_t b1_newb = ShiftBit((arq->Reg[instruction->z]),0,8);
				b1 = b1_newb;
				arq->term = (b4 << 24) + (b3 << 16) + (b2 << 8) + b1;
				arq->buffer[(arq->index)] = b1;
				arq->index++;
			//	printf("-> 0x%02X\n",b1);
				if(arq->index == 100){
					arq->buffer = realloc(arq->buffer,sizeof(int32_t)*arq->size + sizeof(int32_t)*100);
					arq->size = arq->size +100;
				}
		...
		//	printf("0x%08X\n",arq->Reg[29]);
			if(((arq->Reg[instruction->x] + IplusSinal) << 2) == 0x80808080){
				arq->watchCount = ShiftBit(arq->Reg[instruction->z],0,30);
				arq->setWatchdog =  ShiftBit(arq->Reg[instruction->z],31,1);
...
void processFile(FILE* input,FILE* output){
	...
	while(exec){
		...
		//	printf("0x%08X\n",newArq->Mem[newArq->Reg[29]]);
			newArq->Reg[29] = 0x00000010;
		} else {
			if(newArq->setWatchdog == 1){
				//printf("%d\n",newArq->watchCount);
				newArq->watchCount--;
			}
		//	printf("->%d\n",newArq->setWatchdog);
...
if(newArq->index > 0){
	...
	for(int i = 0;i < newArq->index;i++){
		fprintf(output,"%c",newArq->buffer[i]);
	}
	fprintf(output,"\n");
}
