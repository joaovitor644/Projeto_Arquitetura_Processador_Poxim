...
void executionInstructionTypeU(ArqResources* arq,typeU* instruction){
...
void executionInstructionTypeF(ArqResources* arq,typeF* instruction){
	char instrucao[30];
	switch(instruction->opcode){
		uint32_t IplusSinal; 
		case 0b011010:
		...
		case 0b100001:
			if(instruction->i == 0){
				...
				printf("0x%08X:\t%-25s\t%s=%s\n",arq->Reg[29],instrucao,whyIsReg(instruction->z,2),whyIsRegHex(instruction->z));
			}else{
...
void executionInstructionTypeS(ArqResources* arq,typeS* instruction){
	char instrucao[30];
	switch(instruction->opcode){
		uint32_t IplusSinal;
		case 0b110111:
...
void executeProgram(ArqResources* newArq,FILE* input){
	...
	while(exec){
		...
		if(optype == 'U'){
			...
			executionInstructionTypeU(newArq,newIU);
			...
			executionInstructionTypeF(newArq,newIF);
		}
		else if(optype == 'S'){
			newIS->opcode = opcode;
